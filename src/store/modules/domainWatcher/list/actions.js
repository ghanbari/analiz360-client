...
export const getItems = ({ commit }, options) => {
  ...
  const request = () => {
    ...
    return fetch('domain_watchers', options)
      ...
      .then((data) => {
        ...
        storage.setItem(`domain_watchers?${JSON.stringify(query)}`, data['hydra:member'], 3600)
  ...
  return storage.getItem(`domain_watchers?${JSON.stringify(query)}`, reload, request)
}
